...
public class CategoriaController {
    ...
    @PostMapping
    public ResponseEntity<RespuestaBase<Categoria>> crear(@Valid @RequestBody Categoria categoria) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<RespuestaBase<Categoria>> actualizar(@PathVariable Long id, @Valid @RequestBody Categoria categoria) {
        categoria.setId(id);
        RespuestaBase<Categoria> respuesta = categoriaService.guardar(categoria);
        if (!respuesta.isExito()) {
            return ResponseEntity.badRequest().body(respuesta);
